#ifndef ISEKAI_HOST_FALCON_RUE_FORMAT_DNA_C_H_
#define ISEKAI_HOST_FALCON_RUE_FORMAT_DNA_C_H_
...
// Changes from DNA_B:
// - eack_drop and eack bits are added.
struct __attribute__((packed)) Event_DNA_C {
  // cid
...
struct __attribute__((packed)) Response_DNA_C {
  // cid
  uint32_t connection_id : kConnectionIdBits;
  // randomize_path
...
static_assert(sizeof(Event_DNA_C) == ABSL_CACHELINE_SIZE,
              "Event is not one cache line :(");
static_assert(sizeof(Response_DNA_C) == ABSL_CACHELINE_SIZE,
              "Response is not one cache line :(");

}  // namespace falcon_rue

#endif  // ISEKAI_HOST_FALCON_RUE_FORMAT_DNA_C_H_
