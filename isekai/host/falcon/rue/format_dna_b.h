#ifndef ISEKAI_HOST_FALCON_RUE_FORMAT_DNA_B_H_
#define ISEKAI_HOST_FALCON_RUE_FORMAT_DNA_B_H_
...
// Changes from DNA_A:
// - event and response fields are rearranged.
// - ecn_counter field is changed to reserved.
// - ecn_accumulated is added.
struct __attribute__((packed)) Event_DNA_B {
  // cid
...
struct __attribute__((packed)) Response_DNA_B {
  // cid
  uint32_t connection_id : kConnectionIdBits;
  // randomize_path
...
static_assert(sizeof(Event_DNA_B) == ABSL_CACHELINE_SIZE,
              "Event is not one cache line :(");
static_assert(sizeof(Response_DNA_B) == ABSL_CACHELINE_SIZE,
              "Response is not one cache line :(");

}  // namespace falcon_rue

#endif  // ISEKAI_HOST_FALCON_RUE_FORMAT_DNA_B_H_
