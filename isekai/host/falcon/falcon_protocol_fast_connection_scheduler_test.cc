...
class FastConnectionSchedulerTest
 ...
 protected:
  void InitFalcon(const FalconConfig& config) override {
    FalconConfig config_with_fast_scheduler = config;
    config_with_fast_scheduler.set_scheduler_variant(
        FalconConfig::EVENT_BASED_SCHEDULER);
    FalconTestingHelpers::FalconTestSetup::InitFalcon(
        config_with_fast_scheduler);
  }
  int GetFalconVersion() { return GetParam(); }
};

INSTANTIATE_TEST_SUITE_P(ConnectionSchedulerTest, FastConnectionSchedulerTest,

                         /*version=*/testing::Values(1),
                         [](const testing::TestParamInfo<
                             FastConnectionSchedulerTest::ParamType>& info) {
...
TEST_P(FastConnectionSchedulerTest, WorkScheduler) {
  FalconConfig config =
...
TEST_P(FastConnectionSchedulerTest, TestMultipleConnections) {
  FalconConfig config =
      DefaultConfigGenerator::DefaultFalconConfig(GetFalconVersion());
...
TEST_P(FastConnectionSchedulerTest, PhantomRequesZeroCost) {
  FalconConfig config =
      DefaultConfigGenerator::DefaultFalconConfig(GetFalconVersion());
...
TEST_P(FastConnectionSchedulerTest, TestOpRateWorkScheduler) {
  FalconConfig config =
      DefaultConfigGenerator::DefaultFalconConfig(GetFalconVersion());
